 ...
 * 本拦截器会拦截CachingExecutor的query方法，在创建cacheKey时将分页参数page包含其中。
 * 
 */
...
public class CacheInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		while (metaExecutor.hasGetter("h")) {//分离代理对象链
		...
		while (metaExecutor.hasGetter("target")) {//分离最后一个代理对象的目标类
	...
	public static void setPageSqlId(String pageSqlId) {
		CacheInterceptor.pageSqlId = pageSqlId;
	}
